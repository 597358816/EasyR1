...
class DataProto:
    ...
    def select_by_index(self, index_list: List[int], deepcopy: bool = False) -> "DataProto":
        """Select a subset of samples in DataProto via sample indices.

        Args:
            index_list (List[int]): list of sample indices to select.
            deepcopy (bool): whether to deep copy non-tensor parts.

        Returns:
            DataProto: a new DataProto containing only the selected samples.
        """
        # === 1. Select batch (tensor part) ===
        sub_batch = self.batch[index_list]
        ...
        # === 3. meta_info不需要按样本索引变化，直接复制 ===
        sub_meta_info = self.meta_info
        if deepcopy:
            sub_meta_info = copy.deepcopy(sub_meta_info)

        # === 4. Return a new DataProto ===
